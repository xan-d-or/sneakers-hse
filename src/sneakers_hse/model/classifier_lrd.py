...
class LitClassifierLRD(pl.LightningModule):
    def __init__(
        self,
        model_name,
        num_classes,
    ...
    def configure_optimizers(self):
        param_groups = self.get_param_groups_lrd(base_lr=self.lr)

        optimizer = torch.optim.AdamW(param_groups)

        return optimizer
    
    def get_param_groups_lrd(self, base_lr=1e-4, weight_decay=0.05, layer_decay=0.75):
        param_groups = []

        layers = []

        layers.append(self.model.stem)
        layers.extend(self.model.stages)
        layers.append(self.model.norm)
